...
def load_subject(path, lp_hz=4.0):
    ...
    return filt.reshape(N_CHAN, N_TRIAL, -1)
...
def main():
    ...
    for wname, (lo, hi) in WINDOWS.items():
        ...
        for ch in range(N_CHAN):
            seg = np.vstack([s[ch, lo:hi, :][::args.decim, :].T for s in cache])
            acc_row[ch] = (decode(seg, y_row, groups)[0] == y_row).mean()
        ...
        print(f"\n        FRONT-5 mean {fro_m * 100:.2f}%   "
              f"OCC-9 mean {occ_m * 100:.2f}%   ratio {fro_m / occ_m:.2f}x")
        ...
        Fm = np.vstack([s[:, lo:hi, :].mean(axis=1).T for s in cache])
